...
HQA_CORPORATE_PROMPT = """You are an expert at generating customer search questions about Royal London as a company.

Given a chunk of text from Royal London's corporate pages (About Us, Our Purpose, Social Impact, AGM etc),
generate exactly 5 questions that a customer or stakeholder would ask about Royal London as an organisation.
...
def score_question(question: str, chunk_content: str) -> int:
    ...
    normalised = _re.sub(r'[?!.,;:]+$', '', normalised).strip()
...
def generate_hqa_questions(
    ...
    system_prompt = (
        HQA_CORPORATE_PROMPT
        if content_type == "corporate"
        else HQA_SYSTEM_PROMPT
    )
...
def deduplicate_questions_across_chunks(
    ...
    Args:
        chunks: List of all chunks with augmented_questions set.
                Modified in-place.

    Returns:
        Same chunks list with deduplicated augmented_questions.
    """
    import re as _re
    ...
    for ci, chunk in enumerate(chunks):
        aq = chunk.get("augmented_questions", "") or ""
    ...
    for norm_q, entries in collisions.items():
        ...
        for ci, priority, original_q in sorted_entries[1:]:
            chunk = chunks[ci]
            aq    = chunk.get("augmented_questions", "") or ""
            qs    = [q.strip() for q in aq.split("\n") if q.strip()]
...
def augment_chunks_with_hqa(
    ...
    # v4.0.0 Fix 3: Cross-chunk deduplication
    # Only runs after ALL chunks are augmented (not in pilot mode).
    # Removes questions appearing in > MAX_COLLISION_THRESHOLD
    # chunks, keeping only the most relevant occurrence.
