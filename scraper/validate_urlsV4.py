...
Usage:
    python scraper/validate_urlsV4.py
    python scraper/validate_urlsV4.py --file path/to/custom.xlsx
    python scraper/validate_urlsV4.py --concurrency 20 --timeout 10
    python scraper/validate_urlsV4.py --retry-timeouts
    python scraper/validate_urlsV4.py --retry-timeouts --retry-timeout 25
...
v1.3.0 (V3) — Aug 2026 | Mukesh Kund
         FIX — last-segment internal redirects missed (false
         "Live" result).

         BUG: /pension-tools/retirement-income-and-lifestyle-planner
         returns 200 and redirects to
         /pension-tools/retirement-planner — a genuinely
         different page (content removed, replaced by a
         differently-named tool page). Confirmed independently
         by content_freshnessV1.py's stricter exact-path health
         check, which correctly flagged it as internal_redirect
         on the same URL.

         ROOT CAUSE: old Signals 1–3 all missed it.
           - Signal 1 (parent path): final path is a SIBLING,
             not an ancestor, of the original — doesn't apply.
           - Signal 2 (50% segment-overlap threshold): 3 of 4
             path segments still matched (guides-tools/
             pension-guides/pension-tools/) = 75% overlap,
             comfortably above the 50% cutoff — didn't fire.
           - Signal 3 (title fuzzy match, threshold 60):
             rapidfuzz.partial_ratio("Retirement income and
             lifestyle planner", "Retirement Planner") = 77.4%
             — both titles share "Retirement"/"Planner", so
             partial_ratio scored well above the cutoff.
         Every signal was checking overall similarity, but the
         LAST path segment — the part that actually identifies
         which page this is — differed completely.

         FIX — NEW SIGNAL 2 (last-segment identity check),
         inserted between the old Signal 1 and Signal 2 (now
         renumbered Signal 3), and made DEFINITIVE like Signal 1:
         If the final URL's last path segment differs from the
         original's last path segment, compare them with
         rapidfuzz.ratio() (not partial_ratio — partial_ratio
         rewards substring containment, which is exactly what
         let "Retirement Planner" slip past "Retirement income
         and lifestyle planner"; plain ratio penalises length
         mismatch, which is what we want here).
         Threshold: < 65% → flag as internal_redirect.

         THRESHOLD VALIDATION (tested against rapidfuzz.ratio,
         last-segment-only comparisons):
           retirement-income-and-lifestyle-planner
             vs retirement-planner            → 63.2%  (FLAG — correct, this is the bug)
           make-a-will vs making-a-will        → 83.3%  (no flag — cosmetic variant, correct)
           pension-tools vs pensiontools       → 96.0%  (no flag — cosmetic, correct)
           lump-sum-calculator vs lumpsum-calculator → 97.3% (no flag — correct)
           is-it-worth-paying-into-a-workplace-pension
             vs is-it-worth-paying-into-workplace-pension → 97.6% (no flag — correct)
         65% sits in a clean gap between the confirmed bug case
         (63.2%) and the nearest legitimate cosmetic variant
         found in the approved-URL set (70.6%+), so it catches
         the real redirect without re-introducing false positives
         on minor wording/hyphenation differences.

         FALLBACK (rapidfuzz not installed): falls back to a
         plain substring check — flag if last segments differ
         AND neither is a substring of the other. Less precise
         than the ratio score but still catches the reported bug
         (neither "retirement-planner" nor "retirement-income-
         and-lifestyle-planner" is a substring of the other).

         Old Signal 2 (path segment overlap < 50%) and old
         Signal 3 (title mismatch) are UNCHANGED and renumbered
         3 and 4 — they remain as softer, supporting signals for
         redirects that change several segments but keep the
         last one recognisable.

         Reason string for the new signal is written to the
         "Issue / Notes" column same as the other signals, e.g.:
         "Final page slug changed significantly (63% match):
         'retirement-income-and-lifestyle-planner' →
         'retirement-planner' at <final_url>"

         NOTE: this signal only ever runs on URLs that already
         produced a genuine HTTP redirect to a different path
         (the calling code's rstrip("/") trigger check is
         unchanged) — it does not affect same-page trailing-
         slash/case-only "transparent" redirects, which are
         still correctly left as "live" by the existing
         orig_path == final_path early-return.

v1.4.0 (V4) — Sep 2026 | Mukesh Kund
         FIX — architectural change: signals are now DESCRIPTIVE,
         not GATING. Any genuine path change is always flagged.

         SECOND MISS FOUND (after V3 shipped):
         /isa/stocksandsharesisa → redirects to
         /isa/stocks-shares-isa/ — a pure hyphenation change,
         but the approved-list URL no longer resolves to itself.
         V3's Signal 2 scored the last-segment similarity at
         rapidfuzz.ratio("stocksandsharesisa", "stocks-shares-isa")
         = 85.7% — comfortably above the 65% threshold meant to
         protect against exactly this kind of cosmetic false
         positive. V3's Signal 3 (segment overlap) also missed
         it: only 1 of 2 segments matched exactly = 50%, not
         strictly below the 50% cutoff (boundary miss).

         WHY A THRESHOLD FIX ISN'T ENOUGH:
         Two real misses in a row, each passing a different
         signal for a different reason, is proof the underlying
         design is wrong, not that a threshold needs tuning.
         Hyphenation alone can push lexical similarity anywhere
         from ~63% (retirement-income-and-lifestyle-planner vs
         retirement-planner) to ~86% (stocksandsharesisa vs
         stocks-shares-isa) depending on word length — there is
         no similarity cutoff that reliably separates "harmless
         reformatting" from "meaningful redirect", because
         reformatting and genuine renames both change spelling
         by similar amounts of edit distance.

         ROOT CAUSE (design-level, not a bug in the maths):
         V2/V3 treated "the redirect destination looks similar"
         as a proxy for "this redirect is safe to ignore". But
         this script's actual purpose (v1.2.0) is narrower and
         stricter than that: the approved-URL list is the single
         source of truth for what may be scraped/indexed (FCA
         compliance boundary — content outside the approved list
         must never be indexed regardless of similarity to
         approved content). A URL that no longer resolves to
         itself has drifted from that list. Whether the new
         destination happens to look similar doesn't change the
         fact that the list entry is stale and needs a human
         decision — it only changes how urgently.
         content_freshnessV1.py already uses this stricter
         catch-all model (any normalised path difference =
         internal_redirect) and never missed either case.

         FIX: is_internal_redirect() now flags EVERY genuine
         path difference (orig_path != final_path after the
         existing case/trailing-slash normalisation). The 4
         signals from V3 are kept, but repurposed as DESCRIPTORS
         only — they build the most informative "reason" string
         (parent-redirect / slug-rename with a %-match shown as
         context / path-restructure / title-mismatch / generic
         fallback) but no signal, high or low, gates whether the
         row gets flagged.

         BEHAVIOUR CHANGE: Action Required will now include
         redirects that are pure cosmetic reformatting (e.g.
         hyphenation, pluralisation) that V2/V3 silently passed
         as "live". This is intentional — the sheet's purpose is
         "does this approved URL still resolve to itself", and a
         cosmetic redirect still answers that "no". The reason
         string flags these as "likely just reformatted" (≥80%
         slug match) so a reviewer can triage them quickly
         without them being silently absorbed as never having
         happened.

         VERIFIED — both known misses now caught, and prior
         true positives (parent redirects, genuine content
         changes) still caught with unchanged reasons:
           retirement-income-and-lifestyle-planner
             → retirement-planner            → FLAGGED (63% match,
                                                 "verify content unchanged")
           stocksandsharesisa → stocks-shares-isa → FLAGGED (86% match,
                                                 "likely just reformatted")
           /insurance/life-insurance/do-more-digitally
             → /insurance/                    → FLAGGED (parent redirect,
                                                 unchanged from V2/V3)

         Old file validate_urlsV3.py kept as-is for
         rollback; no shared state between versions.
...
def is_internal_redirect(
    ...
    V4 — CATCH-ALL REDESIGN (was: 4 similarity-gated signals).

    WHY V2/V3's gated-signal design was wrong:
    The whole point of this function (v1.2.0) is that the
    approved-URL list is the single source of truth for what
    may be scraped/indexed (FCA compliance boundary). ANY URL
    that no longer resolves to itself has drifted from that
    list and needs a human to review it — regardless of how
    similar the destination looks. V2/V3 used similarity scores
    (segment overlap %, last-segment ratio, title match) as
    GATES: if the redirect "looked similar enough", it silently
    passed through as "live". That is backwards — a redirect
    that looks similar is not proof it's the same content, it's
    just a redirect the heuristic couldn't tell apart from noise.

    Two real misses were found this way:
      - retirement-income-and-lifestyle-planner → retirement-planner
        (last-segment ratio 63% happened to sit under some
        thresholds and over others depending on signal — fragile)
      - stocksandsharesisa → stocks-shares-isa
        (pure hyphenation: ratio 85.7%, comfortably "similar" by
        any reasonable threshold, yet the URL genuinely redirects
        elsewhere and the approved list entry is stale)
      Both are proof that lexical similarity cannot reliably
      distinguish "harmless reformatting" from "meaningful
      redirect" — hyphenation alone can swing the score either
      way. There is no safe threshold.

    V4 FIX: any genuine path difference is flagged, full stop.
    The signals below are now purely DESCRIPTIVE — they build
    the most useful "reason" string for the reviewer, but no
    signal (or absence of one) changes whether flagging happens.
    This matches content_freshnessV1.py's simpler, stricter
    model, which never missed either case above.

    Descriptive signals (checked in order, first match wins):

    1. PARENT PATH: final path is a direct ancestor of the
       original → "Page removed — redirects to parent".
    2. SLUG CHANGE: last path segment differs → reason names
       old and new slug, with a rapidfuzz.ratio() % shown as
       context only (e.g. "45% match" or "89% match, likely
       reformatting") — informational, never a gate.
    3. PATH RESTRUCTURE: last segment matches but an earlier
       segment changed → reason shows % of original segments
       still present.
    4. TITLE MISMATCH: nothing else distinguished it, but the
       fetched live title differs from the Excel title.
    5. Fallback: generic "Redirects to a different URL".
    ...
    # No redirect at all (covers trailing-slash/case-only diffs,
    # which never reach here since both sides are normalised)
    if orig_path == final_path:
        return False, ""

    # Every remaining case below is a genuine path change — V4
    # always returns True from here on. Only the reason text
    # varies by which descriptive signal matches best.

    # Descriptive signal 1 — parent path (most useful framing)
    if orig_path.startswith(final_path + "/") and len(final_path) < len(orig_path):
        return True, f"Page removed — redirects to parent: {final_url}"

    orig_segments  = [s for s in orig_path.split("/") if s]
    final_segments = [s for s in final_path.split("/") if s]

    last_orig  = orig_segments[-1] if orig_segments else ""
    last_final = final_segments[-1] if final_segments else ""

    # Descriptive signal 2 — slug change (most common case)
    if last_orig and last_final and last_orig != last_final:
        if _RAPIDFUZZ_AVAILABLE:
            seg_score = round(_fuzz.ratio(last_orig, last_final))
            qualifier = (
                "likely just reformatted" if seg_score >= 80
                else "verify content unchanged" if seg_score >= 60
                else "likely different content"
            )
            return True, (
                f"Final page slug changed ({seg_score}% match, "
                f"{qualifier}): '{last_orig}' → '{last_final}' "
                f"at {final_url}"
            )
        return True, (
            f"Final page slug changed: '{last_orig}' → "
            f"'{last_final}' at {final_url}"
        )

    # Descriptive signal 3 — earlier segment(s) changed, last
    # segment happens to match (e.g. section moved but page
    # kept its own slug)
    if orig_segments:
        common = sum(1 for s in final_segments if s in orig_segments)
        similarity = round(common / len(orig_segments) * 100)
        if similarity < 100:
            return True, (
                f"Path restructured ({similarity}% segment overlap) "
                f"— redirects to: {final_url}"
            )

    # Descriptive signal 4 — title mismatch (rare fallback: path
    # segments identical in content but something else changed,
    # e.g. locale/case already normalised out above)
    if (
        ...
        if clean_orig and clean_live:
            score = round(_fuzz.partial_ratio(
    ...
    # Fallback — genuine path diff, no stronger descriptor found
    return True, f"Redirects to a different URL: {final_url}"
    ...
    if args.file:
        ...
        if not excel_path:
            print(
                ...
                f"   python scraper/validate_urlsV4.py --file path/to/file.xlsx\n"
