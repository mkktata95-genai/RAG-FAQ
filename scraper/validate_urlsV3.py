...
Usage:
    python scraper/validate_urlsV3.py
    python scraper/validate_urlsV3.py --file path/to/custom.xlsx
    python scraper/validate_urlsV3.py --concurrency 20 --timeout 10
    python scraper/validate_urlsV3.py --retry-timeouts
    python scraper/validate_urlsV3.py --retry-timeouts --retry-timeout 25
...
def is_internal_redirect(
    ...
    FOUR signals (any one is sufficient to flag):
    ...
    Signal 2 — LAST-SEGMENT IDENTITY CHECK (strong — V3):
    The last path segment is what actually identifies the page.
    Old Signals 3/4 (overall segment overlap, full-title fuzzy
    match) can both score "similar enough" even when the final
    page is genuinely different content, because a shared prefix
    (e.g. /pension-tools/) and shared words in the title inflate
    the score. This signal isolates just the last segment and
    scores original vs. final with rapidfuzz.ratio() (whole-
    string similarity, not partial_ratio — partial_ratio rewards
    substring containment, which is exactly the failure mode:
    "retirement-planner" scores as a good partial match inside
    "retirement-income-and-lifestyle-planner"). ratio() penalises
    the length/content mismatch instead.
    Threshold: < 65% match → flag as internal_redirect.
    e.g. retirement-income-and-lifestyle-planner vs
    retirement-planner → 63.2% → flagged.
    Cosmetic variants (make-a-will vs making-a-will = 83.3%,
    pension-tools vs pensiontools = 96.0%) stay above threshold
    and are correctly left unflagged by this signal.
    If rapidfuzz is unavailable, falls back to a plain substring
    check: flag if segments differ and neither contains the
    other.

    Signal 3 — PATH SEGMENT SIMILARITY (< 50%):
    ...
    Signal 4 — PAGE TITLE MISMATCH (supporting):
    ...
    # Signal 2 (V3) — last-segment identity check
    # The last segment is the page's real identity. A redirect
    # that only renames it (while the parent path stays intact)
    # otherwise slips past Signal 3's overall-overlap check,
    # since most of the path still "matches".
    if orig_segments and final_segments:
        last_orig  = orig_segments[-1]
        last_final = final_segments[-1]
        if last_orig != last_final:
            if _RAPIDFUZZ_AVAILABLE:
                seg_score = _fuzz.ratio(last_orig, last_final)
                if seg_score < 65:
                    return True, (
                        f"Final page slug changed significantly "
                        f"({round(seg_score)}% match): "
                        f"'{last_orig}' → '{last_final}' "
                        f"at {final_url}"
                    )
            else:
                # No rapidfuzz — cruder fallback, still catches
                # the reported bug (neither segment is a substring
                # of the other).
                if last_orig not in last_final and last_final not in last_orig:
                    return True, (
                        f"Final page slug changed: "
                        f"'{last_orig}' → '{last_final}' "
                        f"at {final_url}"
                    )

    # Signal 3 — path segment similarity
    if orig_segments:
    ...
    # Signal 4 — title mismatch (supporting only)
    if (
    ...
    if args.file:
        ...
        if not excel_path:
            print(
                ...
                f"   python scraper/validate_urlsV3.py --file path/to/file.xlsx\n"
