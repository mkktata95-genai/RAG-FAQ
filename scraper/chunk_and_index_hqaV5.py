"""
Royal London FAQ - Chunk and Index (v5 — element-aware chunking)
Chunks scraped content, generates HQA-augmented embeddings
plus broad entry-point title_questions, pushes to Azure AI
Search. Targets rlg-faq-index-v5 — rlg-faq-index-v4 and
rlg-faq-index-v4-baseline are untouched and remain live.

WHY v5 (not a re-run of v4):
  v4/v4-baseline chunk standard pages with a flat
  RecursiveCharacterTextSplitter — confirmed via Phase 0/1 audit
  to split 18 of 31 table pages mid-row, and to let tab/FAQ-style
  ##/### header sections bleed into each other when combined
  content exceeds CHUNK_SIZE. v5 introduces element-aware chunking
  (element_detector.py, shared with content_freshness.py) — tables
  chunked atomically, headers treated as hard section boundaries.
  Rather than risk the live v4 index, v5 is a clean build so v4
  stays live as fallback until v5 is validated in production.
...
Supports:
  ...
  --no-hqa:   Baseline mode — skip ALL LLM calls (no HQA
              ...
              rlg-faq-index-v5-baseline for clean A/B comparison
              against the full HQA index (rlg-faq-index-v5).
...
Usage:
    # Full HQA run — creates rlg-faq-index-v5
    python scraper/chunk_and_index_hqaV5.py --full

    # Baseline run — no LLM calls — creates rlg-faq-index-v5-baseline
    python scraper/chunk_and_index_hqaV5.py --full --no-hqa

    # Pilot — validate HQA quality on first 100 chunks only
    python scraper/chunk_and_index_hqaV5.py --full --pilot

    # Dry run — validate config + chunking, no index changes
    python scraper/chunk_and_index_hqaV5.py --full --dry-run

    # Custom file
    python scraper/chunk_and_index_hqaV5.py --full --file path/to/file.json

Programmatic (DevOps / Container Apps Job):
    from scraper.chunk_and_index_hqaV5 import run_pipeline
...
# TODO (DevOps): Create Container Apps Job: digital-assistance-indexer-job
# Trigger: manual only (ADO pipeline, after digital-assistance-scraper-job completes)
...
#   COPY . .
#   CMD ["python", "scraper/chunk_and_index_hqaV5.py", "--full"]
#
...
#   TODO (DevOps): assign identity to digital-assistance-indexer-job + grant roles.
#
...
#
#   # Manual trigger — ADO pipeline after digital-assistance-scraper-job:
#   az containerapp job start \
#       --name digital-assistance-indexer-job \
#       --resource-group <rg>
#
...
#   2     digital-assistance-indexer-job    --full --no-hqa → v5-baseline (~20 min)
#   3     digital-assistance-indexer-job    --full → v5 full HQA (~3.5 hrs)
#   4     DevOps (Key Vault)                AZURE-SEARCH-INDEX-NAME → rlg-faq-index-v5
#   5     Digital Assistance server         Restart → picks up new index from Key Vault
#   6     digital-assistance-freshness-job  --mode report → verify hashes + URLs
#   7     digital-assistance-freshness-job  Enable nightly schedule (0 2 * * *)
#   8     Monitor                           Keep v4 live for 1-2 weeks as fallback
#
# ── IMPORTANT NOTES ─────────────────────────────────────────────────
#
#   - NEVER run --full on rlg-faq-index-v4 or v4-baseline.
#     These are the live fallback indexes. Wiping them removes
#     your rollback option. v5 targets only.
...
v5.8.3 — July 2026 | Mukesh Kund
         ...
         - GPT-5 reasoning models return prose preamble or postamble
           around the JSON array (e.g. "Here are the questions: [...]").
           The previous startswith("```") fence-strip failed on these,
           causing hqa_json_parse_failed on every chunk.
...
v5.8.6 — July 2026 | Mukesh Kund
         ...
         augment_chunks_with_hqa() [MODIFIED]:
         - Est. time and cost now vary by model family:
             gpt-4o-mini: ~2.4s/chunk, $0.000116/question (original)
             gpt-5-*:     ~12s/chunk,  $0.547/100 chunks
                          (measured from pilot run 17-Jul-2026)
         - Allows operators to anticipate run duration and cost
           before committing to a full re-index.
...
v5.10.0 — August 2026 | Mukesh Kund
         RENAMED: chunk_and_index_hqaV4.py → chunk_and_index_hqaV5.py.
         Targets rlg-faq-index-v5 / rlg-faq-index-v5-baseline.
         rlg-faq-index-v4 and rlg-faq-index-v4-baseline are NOT
         touched by this script — both remain live/untouched in
         Azure AI Search as fallback during the v5 rollout.

         ELEMENT-AWARE CHUNKING (new element_detector.py module):
         Confirmed via Phase 0/1 audits on the 297-URL approved list:
           - 31 pages contain tables; 18 would be split mid-row by
             the old flat splitter (max observed: 96 rows across 16
             tables on one page — retirement-living-standards).
           - 12 pages use content tabs (Invest/Engage/Embed style) —
             standard crawl4ai scrape already captures all tabs as
             ### <label> markdown headers; no scraper change needed,
             only the chunker needed to respect header boundaries.
           - 4 confirmed FAQ-accordion pages with substantial content
             per answer (pensions-explained: 53,918 chars / 8
             questions, ~6,740 chars/answer — well over CHUNK_SIZE,
             ruling out atomic per-answer treatment).

         chunk_pages() [MODIFIED]:
         - Standard (non-dropdown_state) pages now call
           chunk_content_element_aware() from the new shared
           element_detector.py module instead of calling
           RecursiveCharacterTextSplitter.split_text() directly.
         - element_detector.py is imported by BOTH this file and
           content_freshness.py — single source of truth. Fixes
           duplication-drift risk found during this audit: both
           files had independently re-implemented clean_content(),
           compute_content_hash(), AND the whole chunking function
           (confirmed byte-identical at audit time, but with nothing
           preventing future drift).
         - Behaviour for the 296/297 pages with no tables/headers is
           UNCHANGED — element_detector.py's fast-path produces
           byte-identical output to the old flat splitter for these.
           Only table pages and header-containing pages are affected.
         - New "element_type" field added to every chunk dict
           ("prose" | "table" | "dropdown_state") for audit
           traceability — lets ops query how many chunks in the
           index came from table vs prose content without re-parsing
           raw scraped content.

         RecursiveCharacterTextSplitter is no longer imported
         directly in this file — element_detector.py owns the
         splitter instance internally.

         PIPELINE_VERSION bumped 1.0.0 → 1.1.0 (chunking logic
         changed — requires --full reindex, per existing bumping
         rules documented in the constants section).

         NOT IN SCOPE (deliberately, confirmed via audit before
         building anything):
           - Accordion-specific detection/chunking code — the same
             header-boundary mechanism used for tabs covers FAQ-style
             accordion pages too. No separate "accordion" code path.
           - fund-prices JS-pagination handling — still parked
             pending client confirmation on whether that URL stays
             in scope.
...
# v5.10.0 — element-aware chunking, shared with content_freshness.py
# (single source of truth — see element_detector.py header for the
# duplication-drift issue this fixes). RecursiveCharacterTextSplitter
# is no longer imported directly here — element_detector.py owns the
# splitter instance internally.
...
#   Set by scrape_approved_urls_updatedV5.py SCRAPER_VERSION.
...
#   Set by scrape_approved_urls_updatedV5.py METADATA_VERSION.
#   Default "unknown" for pre-v4.6.0 scrape JSON files.
...
#   Set by scrape_approved_urls_updatedV5.py at startup.
#   Default "unknown" for pre-v4.6.0 scrape JSON files.
...
#   Set by scrape_approved_urls_updatedV5.py or content_freshnessV1.py.
...
#     refresh_count
#
# v5.10.0: bumped 1.0.0 → 1.1.0 — element-aware chunking changes HOW
# standard pages are chunked (table atomicity, header boundaries).
# Lets audit scripts distinguish v4-index chunks (flat splitter) from
# v5-index chunks (element-aware) via this field alone.
...
# v5.10.0: default changed to rlg-faq-index-v5.
# rlg-faq-index-v4 and rlg-faq-index-v4-baseline are NOT touched
# by this script — both remain live as fallback during the v5
# element-aware-chunking rollout.
# Set AZURE_SEARCH_INDEX_NAME explicitly to override.
INDEX_NAME            = os.getenv("AZURE_SEARCH_INDEX_NAME", "rlg-faq-index-v5")

# v5.10.0: baseline index name updated to v5-baseline.
# Targets rlg-faq-index-v5-baseline when --no-hqa flag is set.
...
def clean_content(text: str) -> str:
    ...
    citations in responses — making it look like Digital Assistance is
...
def chunk_pages(pages: list[dict]) -> list[dict]:
    ...
    v5.10.0 — ELEMENT-AWARE CHUNKING (standard pages only):
    Standard pages now go through chunk_content_element_aware()
    (element_detector.py, shared with content_freshness.py) instead of
    a flat RecursiveCharacterTextSplitter call. Tables are chunked
    atomically (row-capped); ##/### headers act as hard section
    boundaries so a chunk never bleeds across a topic change (tab
    pages, FAQ-accordion-style pages). Pages with no tables/headers
    get byte-identical output to the old flat splitter — see
    element_detector.py's fast-path for the 296/297 pages unaffected
    by this change.
    """
    ...
    for page in pages:
        ...
        if is_dropdown_state:
            ...
            if len(content_with_title.strip()) >= 50:
                chunks.append({
                    ...
                    # v5.10.0: element_type — dropdown_state pages are
                    # always atomic contact-detail content, never
                    # table/prose-split. Consistent field across all
                    # chunk types for audit traceability.
        ...
        # Standard page — v5.10.0: element-aware chunking.
        # Replaces the flat splitter.split_text() call. For pages with
        # no headers/tables (296/297 in current scope) this produces
        # IDENTICAL output to the old flat splitter — see
        # element_detector.py's fast-path. Behaviour only changes for
        # pages with tables (atomic, row-capped) or ##/### headers
        # (hard section boundaries, never bleeding into each other).
        #
        # v5.10.1 — BUGFIX: total_chunks must be computed AFTER
        # filtering sub-50-char fragments, not before. The original
        # pattern (present since v5.x) set total_chunks from the
        # pre-filter piece count, so a dropped small fragment left
        # surviving chunks with a total_chunks value that didn't match
        # their actual count. Rare with the old flat greedy splitter;
        # more likely with element-aware chunking, since header-
        # boundary splitting naturally produces short leading/trailing
        # fragments (e.g. a one-line closing sentence after a table).
        # Caught by content_freshnessV1.py's validate_chunking_
        # preflight() during testing — fixed here to match.
        pieces       = chunk_content_element_aware(
            content_with_title, CHUNK_SIZE, CHUNK_OVERLAP
        )
        ...
        for i, piece in enumerate(valid_pieces):
            ...
            chunks.append({
                ...
                # v5.9.0: clean parent URL for dropdown state chunks
                "parent_url":           page.get("parent_url", ""),
                # v5.10.0: element_type — "prose" or "table", set by
                # element_detector.py. Lets audit scripts count how
                # many chunks came from table vs prose content without
                # re-parsing raw content.
...
# Question to ask: "Should Digital Assistance answer general money-saving /
...
def deduplicate_questions_across_chunks(
    ...
    def normalise(q: str) -> str:
        ...
        # so near-duplicate questions collapse to the same key.
        q = q.lower().strip()
        q = re.sub(r'[?!.,;:]+$', '', q)
        return ' '.join(q.split())
...
def augment_chunks_with_hqa(
    ...
    # v5.8.6: time/cost estimate is model-aware.
    # gpt-4o-mini: ~2.4s/chunk, $0.000116/question (original estimate)
    # gpt-5-mini:  ~12s/chunk, ~$0.00547/chunk (measured from pilot run)
    _is_gpt5 = "gpt-5" in HQA_DEPLOYMENT.lower()
    if _is_gpt5:
        est_time_min = round(total * 12 / 60) + 1
        est_cost     = (total / 100) * 0.547   # $0.547 per 100 chunks (pilot actuals)
        cost_note    = f"~${est_cost:.2f} (gpt-5 pilot actuals: $0.547/100 chunks)"
    else:
        est_time_min = total // 30 + 1
        est_cost     = est_questions * 0.000116
        cost_note    = f"~${est_cost:.2f}"
    print(
        f"   Est. questions: ~{est_questions:,} | "
        f"Est. cost: {cost_note} | "
        f"Est. time: ~{est_time_min} min"
    )
...
def create_or_update_index(fresh: bool = False):
    ...
    fields = [
        ...
        # filterable: scope Digital Assistance queries to specific product areas.
        ...
        # v5.10.0: element_type — "prose" | "table" | "dropdown_state".
        # Set by element_detector.py (chunk_content_element_aware) for
        # standard pages, or hardcoded "dropdown_state" for dropdown
        # atomic chunks. Filterable so ops can query e.g.
        # "element_type eq 'table'" to audit table-chunk coverage
        # without re-parsing raw scraped content.
        SimpleField(
            name="element_type",
            type=SearchFieldDataType.String,
            searchable=False,
