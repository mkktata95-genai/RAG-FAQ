...
# ── Step 2b: Multi-state dropdown helpers (v4.4.0) ─────

# JS injected into the crawl4ai page to detect <select> elements
# that act as routing dropdowns (more than one non-placeholder option).
...
# JS template: select an option by text and fire DOM events so
# Royal London's event listeners update the visible contact panel.
_JS_SELECT_OPTION = """
(args) => {
    const selects = Array.from(document.querySelectorAll('select'));
    const sel = selects[args.selectIndex];
    if (!sel) return false;
    const target = Array.from(sel.options)
        .find(o => o.text.trim() === args.optionText);
...
# JS to grab the full visible body text after a dropdown change.
_JS_GET_BODY_TEXT = "() => document.body.innerText"


def _diff_dropdown_content(default_lines: set[str], new_text: str) -> str:
    """
    Return only lines present in new_text but NOT in default_lines.

    Purpose: after selecting a dropdown option the page re-renders a
    contact panel (phone number, address, form link). The rest of the
    page is identical to the default state. Diffing extracts only the
    changed content so we don't duplicate shared static text across
    every dropdown entry.

    Args:
        default_lines: set of stripped lines from the default page state.
        new_text:      raw body text after selecting an option.

    Returns:
        Newline-joined string of changed lines (may be empty string).
    """
    changed = [
        line.strip()
...
async def _scrape_dropdown_states(
    ...
    """
    Scrape per-option content from a multi-state dropdown page.

    Called only when _JS_DETECT_DROPDOWNS finds routing <select>
    elements. For each option it:
      1. Injects JS to select the option + fire change events.
      2. Waits 1.5 s for the DOM contact panel to update.
      3. Diffs the new body text against the default state to extract
         only the changed lines (phone numbers, addresses, form links).
      4. Builds a page_data dict with a synthetic URL
         (base_url#policy=<option_value>) so the indexer treats each
         option as a distinct document.

    The base page (default state) is NOT returned here — it is already
    captured by scrape_page() before this function is called.

    Args:
        crawler:        Active AsyncWebCrawler instance.
        url:            Canonical URL of the page.
        base_page_data: page_data dict from the default scrape — used
                        to inherit metadata fields (audience, has_video,
                        content_type, etc.) so we don't re-derive them.
        excel_category: Raw category string from the Excel file.

    Returns:
        List of page_data dicts, one per dropdown option that produced
        changed content. Empty list if JS execution fails or no options
        produce changed content.
    """
    results: list[dict] = []

    try:
        # ── Load the page again for JS interaction ──────────────
        # We need a session where we can inject JS repeatedly.
        # Use js_code to detect dropdowns on first load, then
        # use subsequent arun() calls with js_code to select each option.
        # crawl4ai re-uses the same browser context within one crawler
        # instance so state persists across arun() calls on the same URL.

        # Step 1: detect dropdowns + capture default body text
        ...
        detect_result = await crawler.arun(url=url, config=detect_config)

        if not detect_result.success:
            log.warning(
                "dropdown_detect_failed",
                url=url,
                error=detect_result.error_message,
            )
            return results

        # js_result holds the return value of the last js_code expression
        dropdowns = getattr(detect_result, "js_return_value", None) or []
        if not dropdowns:
            # No routing dropdowns found on re-load — skip
            return results

        # Capture default body text for diffing
        default_text_config = CrawlerRunConfig(
            js_code=_JS_GET_BODY_TEXT,
            wait_until="networkidle",
            page_timeout=30000,
            verbose=False,
        )
        default_text_result = await crawler.arun(url=url, config=default_text_config)
        default_body = getattr(default_text_result, "js_return_value", "") or ""
        ...
        # Step 2: iterate each dropdown × each option
        for dropdown in dropdowns:
            ...
            for option in options:
                ...
                try:
                    # Inject JS: select the option + fire events + wait + get body text
                    # All in one js_code string so crawl4ai executes it atomically.
                    select_and_capture_js = f"""
async () => {{
    const selects = Array.from(document.querySelectorAll('select'));
    ...
    sel.dispatchEvent(new Event('input',  {{ bubbles: true }}));
    sel.dispatchEvent(new Event('change', {{ bubbles: true }}));
    // Wait 1500 ms for DOM update
    await new Promise(r => setTimeout(r, 1500));
    return document.body.innerText;
}}
"""
                    option_config = CrawlerRunConfig(
                        js_code=select_and_capture_js,
                        wait_until="networkidle",
                        page_timeout=30000,
                        verbose=False,
                    )
                    option_result = await crawler.arun(url=url, config=option_config)

                    if not option_result.success:
                        log.warning(
                            "dropdown_option_scrape_failed",
                            url=url,
                            option=opt_text,
                            error=option_result.error_message,
                        )
                        continue

                    new_body = getattr(option_result, "js_return_value", "") or ""
                    if not new_body:
                        log.warning(
                            "dropdown_option_empty_body",
                            url=url,
                            option=opt_text,
                        )
                        continue

                    dynamic_content = _diff_dropdown_content(default_lines, new_body)

                    if not dynamic_content or len(dynamic_content.strip()) < 20:
                        log.warning(
                            "dropdown_option_no_change",
                            url=url,
                            option=opt_text,
                    ...
                    # Build a synthetic URL so the indexer treats this
                    # option as a distinct document. Fragment is not
                    # fetched by browsers so it is safe to append.
                    safe_value = re.sub(r"[^a-zA-Z0-9_-]", "_", opt_value or opt_text)
                    ...
                    page_data = {
                        # Core fields — inherit base page metadata where appropriate
                        "url":            state_url,
                        "title":          f"{base_page_data['title']} — {opt_text}",
                        ...
                        # Enrichment — inherited from base page (same page, same metadata)
...
async def scrape_page(
    ...
    try:
        ...
        # v4.4.0 — detect routing dropdowns on this page.
        # Run a lightweight JS probe against the already-loaded page.
        # If routing <select> elements exist, scrape each option state
        # and return a list so the batch loop can flatten them.
        try:
            dropdown_states = await _scrape_dropdown_states(
                crawler, url, page_data, excel_category
            )
        except Exception as _de:
            log.warning(
                "dropdown_detection_skipped",
                url=url,
                error=str(_de),
            )
            dropdown_states = []
        ...
        if dropdown_states:
            ...
            # Return base page + all option states as a list.
            # Caller (batch loop) must handle list return.
